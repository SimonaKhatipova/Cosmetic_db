...
export default function App() {
  ...
  const groups = [...new Set(ingredients.map(i => i.primaryGroup).filter(Boolean))].sort();

  useEffect(() => {
    restoreSession().then(ok => { setAuthed(ok); setAuthChecked(true); });
  }, []);
  ...
  const loadIngredients = useCallback(async () => {
    ...
    try {
      const data = await sbFetch(
        ...
        "&order=inci_name.asc&limit=6000"
      );
      // вычисляем основную группу (is_primary) для отображения и фильтра
  ...
  const openProduct = async (product) => {
    ...
    try {
      const data = await sbFetch(
        `/product_ingredients?product_id=eq.${product.id}` +
        `&select=*,ingredients(id,inci_name,ru_name,description,is_eu_allergen,` +
        `ingredient_groups(group,subgroup,subgroup2,is_primary),` +
  ...
  const filteredIngredients = ingredients.filter(i => {
    ...
    return matchSearch && (!filterGroup || i.primaryGroup === filterGroup);
  });

  if (!authChecked) {
    return (<><style>{styles}</style><div className="loading" style={{ paddingTop: "20vh" }}>Загрузка...</div></>);
  }
  if (!authed) {
    return (<><style>{styles}</style><LoginScreen onSuccess={() => setAuthed(true)} /></>);
  }
        ...
        <div className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button
              ...
              title="Выйти"
              style={{ background: "rgba(255,255,255,.12)", border: "none", borderRadius: 8, height: 34, padding: "0 12px", cursor: "pointer", fontSize: 12, color: "var(--cream)" }}
            >Выйти</button>
          </div>
        </div>
                    ...
                    {filteredIngredients.map(i => (
                      <tr key={i.id}>
                        <td className="td-inci">
                          {i.inci_name}
                          {i.is_eu_allergen && <span title="Аллерген EU-26" style={{ marginLeft: 6, fontSize: 10, color: "var(--rose)", border: "1px solid #e0b0aa", borderRadius: 4, padding: "1px 4px" }}>EU</span>}
                        </td>
                        <td>{i.ru_name || "—"}</td>
                        <td><span className="ingredient-group td-group">{i.primaryGroup || "—"}</span></td>
                        <td style={{ fontSize: 12, color: "var(--brown)" }}>{i.primarySubgroup || "—"}</td>
        ...
        {selectedProduct && (
          <ProductModal
            ...
            onImageSaved={async (id, url) => {
              ...
              setSelectedProduct(p => p ? { ...p, image_url: url } : p);
...
function ProductModal({ product, ingredients, onClose, onDelete, onImageSaved, removeBgKey }) {
  const [editingImage, setEditingImage] = useState(false);

  const groupOf = (ing) => {
    const gs = ing?.ingredient_groups || [];
    return gs.find(g => g.is_primary) || gs[0] || null;
  };

  const attrs = [
    ["Увлажнение", product.attr_moisture],
    ["Питание", product.attr_nutrition],
    ["Восстановление", product.attr_repair],
    ["Защита", product.attr_protection],
    ["Для кудрей", product.attr_curls],
    ["Плёнка", product.attr_film],
  ].filter(([, v]) => v && String(v).trim() && String(v).trim() !== "0");

  const matched = ingredients.filter(r => r.ingredients).length;
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: "1.2rem" }}>
          {!editingImage ? (
            ...
            <div style={{ border: "1px solid var(--sand)", borderRadius: 10, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <span style={{ fontSize: 12, fontWeight: 500, color: "var(--brown)", textTransform: "uppercase", letterSpacing: ".05em" }}>Изображение</span>
                <button className="btn btn-ghost btn-sm" onClick={() => setEditingImage(false)}>Отмена</button>
              </div>
              <ImagePicker removeBgKey={removeBgKey} onDone={async (url) => {
                if (url) { await onImageSaved(product.id, url); setEditingImage(false); }
              }} />
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
            {product.product_type && <span className="ingredient-group">{product.product_type}</span>}
            {product.skin_type && <span className="ingredient-group">Тип: {product.skin_type}</span>}
            {product.price_rub ? <span className="ingredient-group">{product.price_rub} ₽</span> : null}
          </div>
        </div>

        {attrs.length > 0 && (
          <div style={{ marginBottom: "1.2rem" }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: "var(--brown)", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 8 }}>Действие средства</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {attrs.map(([label]) => (
                <span key={label} style={{ fontSize: 12, padding: "3px 10px", borderRadius: 20, background: "#f0f7f1", color: "var(--sage)", border: "1px solid #a8d4af" }}>{label}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ fontSize: 12, fontWeight: 500, color: "var(--brown)", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 8 }}>
          Состав — {ingredients.length} ингредиентов{ingredients.length > 0 && <span style={{ color: "var(--dust)", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}> · распознано {matched}</span>}
        </div>
        <div className="ingredients-list">
          {ingredients.length === 0
            ...
            : ingredients.map((row, idx) => {
              ...
              const grp = groupOf(ing);
              const oilArr = ing?.oils_meta;
              const oil = Array.isArray(oilArr) ? oilArr[0] : oilArr;
              return (
                <div key={row.id} className="ingredient-row" style={{ alignItems: "flex-start" }}>
                  <div className="ingredient-pos">{row.position || idx + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div className="ingredient-name">
                      {ing?.inci_name || row.raw_inci_name}
                      {ing?.is_eu_allergen && <span title="Аллерген EU-26" style={{ marginLeft: 6, fontSize: 10, color: "var(--rose)", border: "1px solid #e0b0aa", borderRadius: 4, padding: "1px 4px" }}>EU</span>}
                      {!ing && <span title="Не распознан в справочнике" style={{ marginLeft: 6, fontSize: 10, color: "var(--dust)" }}>не в базе</span>}
                    </div>
                    {ing?.ru_name && <div className="ingredient-desc">{ing.ru_name}</div>}
                    {grp?.subgroup && <div className="ingredient-desc" style={{ marginTop: 2 }}>Подгруппа: {grp.subgroup}{grp.subgroup2 ? ` · ${grp.subgroup2}` : ""}</div>}
                    {ing?.description && <div className="ingredient-desc" style={{ marginTop: 2 }}>{ing.description}</div>}
                    {oil && (oil.comedogenicity || oil.penetration || oil.fatty_acids) && (
                      <div className="ingredient-desc" style={{ marginTop: 3, color: "var(--gold)" }}>
                        {oil.comedogenicity && `Комедогенность: ${oil.comedogenicity}. `}
                        {oil.penetration && `Проникновение: ${oil.penetration}. `}
                        {oil.fatty_acids && `Жирные кислоты: ${oil.fatty_acids}.`}
                      </div>
                    )}
                  </div>
                  {grp?.group && <div className="ingredient-group">{grp.group}</div>}
                </div>
              );
            })
          }
...
function LoginScreen({ onSuccess }) {
  ...
  const submit = async () => {
    if (!email.trim() || !password) { setError("Введите почту и пароль"); return; }
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--cream)", padding: "1rem" }}>
      <div style={{ width: "100%", maxWidth: 360, background: "white", border: "1px solid var(--sand)", borderRadius: 16, padding: "2rem" }}>
        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: "var(--deep)", textAlign: "center", marginBottom: 4 }}>INCI<span style={{ color: "var(--dust)" }}>base</span></div>
        <div style={{ fontSize: 13, color: "var(--brown)", textAlign: "center", marginBottom: "1.5rem" }}>Вход для владельца</div>
        {error && <div className="error-msg">{error}</div>}
        <div className="form-group">
          <label className="form-label">Почта</label>
          <input className="form-input" type="email" value={email} autoComplete="username"
        ...
        <button className="btn btn-primary" style={{ width: "100%", marginTop: 6 }} onClick={submit} disabled={busy}>
          {busy ? "Вхожу..." : "Войти"}
        </button>
      </div>
    </div>
