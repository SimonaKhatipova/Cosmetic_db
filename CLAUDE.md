...
```
src/
  App.jsx          # боевое приложение: каталог, кабинет, настройки, вся логика и стили
  Landing.jsx      # публичный лендинг Beauty Helper (стеклянное меню, hero, тарифы)
  DesignPreview.jsx# песочница для дизайна (через `npm run design`), не на проде
  legal.js         # ссылки на юр.документы и реквизиты
  lib/             # вспомогательные модули (robokassa.js и др.)
  main.jsx         # точка входа
  index.css        # базовые стили
public/            # статика (иконки app-icon-*, manifest.json, legal/, flowers/)
...
## ВАЖНО: работа с Git и деплоем

Правила, чтобы не терять изменения и не ломать живой сайт.

**Ветки:**
- `test/full-app` — рабочая ветка с исходным кодом. Здесь ведём разработку, сюда коммитим и пушим. Сейчас это основная ветка проекта.
- `gh-pages` — ветка-артефакт: в неё попадает **только** собранный `dist/` через `npm run deploy`. **Никогда не коммитить в неё руками** и не редактировать её файлы.
- `main`, `design/*` — вспомогательные/исторические, по умолчанию не трогаем.

**Когда коммитить:**
1. После каждой завершённой правки/функции — отдельный коммит. Не копить десятки несохранённых файлов: если работа сделана и проверена — сразу коммит.
2. Один коммит = одна логическая порция изменений. Сообщение — на русском, кратко суть (как в истории: «Личный кабинет + лимиты тарифов…»).
3. Коммит/пуш/деплой делаем, **только когда пользователь об этом просит** (или явно подтвердил). Не пушим молча.

**Перед коммитом — обязательно:**
- `npm run build` проходит без ошибок (не коммитим сломанную сборку).
- Удалён весь временный/превью-код (обходы авторизации, мок-данные, отладка).
- Нет секретов в коде (ключ remove.bg вводится в рантайме и нигде не сохраняется; токены/пароли не коммитим).
- Большое удаление или перезапись файла — сначала посмотреть, что именно удаляешь.

**Порядок «сохранить и выложить»:**
```bash
git add -A                 # или точечно нужные файлы
git commit -m "Суть правок"  # на русском
git push                   # в origin/test/full-app — код сохранён в облаке
npm run build              # проверить сборку
npm run deploy             # собрать dist/ и запушить в gh-pages → живой сайт
```
Живой сайт: https://simonakhatipova.github.io/Cosmetic_db/ (обновляется через 1–2 минуты после `npm run deploy`).

**Правило безопасности:** деплой (`npm run deploy`) публикует сайт в интернет. Делать его только после того, как изменения закоммичены и запушены в `test/full-app`, — чтобы в любой момент можно было откатиться к исходникам.
